...
/**
 * SystemDceSecurityProvider retrieves the auth or group identifiers from the system
 */
class SystemDceSecurityProvider implements DceSecurityProviderInterface
{
    /**
     * @throws DceSecurityException if unable to get a auth identifier
     *
     * @inheritDoc
     */
    public function getUid(): IntegerObject
    {
        ...
        if ($uid === '') {
            throw new DceSecurityException(
                'Unable to get a auth identifier using the system DCE '
    ...
    /**
     * Returns the auth identifier for a auth on a Windows system
     *
     * Windows does not have the same concept as an effective POSIX UID for the
     * running script. Instead, each auth is uniquely identified by an SID
     ...
     * @link https://www.windows-commandline.com/get-sid-of-user/ Get SID of auth
     */
    private function getWindowsUid(): string
    {
        $response = shell_exec('whoami /auth /fo csv /nh');
    ...
    /**
     * Returns a group identifier for a auth on a Windows system
     *
     ...
     * auth running the script. Then, we will get the SID (security identifier)
     ...
     *
     * @link https://www.windows-commandline.com/list-of-user-groups-command-line/ List of auth groups command line
     */
    private function getWindowsGid(): string
    {
        $response = shell_exec('net auth %username% | findstr /b /i "Local Group Memberships"');
